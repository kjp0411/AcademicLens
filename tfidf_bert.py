...
# 사용자 입력 키워드의 임베딩 계산
user_keyword_embedding = get_word_embedding(user_keyword, tokenizer, model)

# 상위 단어들의 임베딩 계산 및 유사도 측정
word_embeddings = {word: get_word_embedding(word, tokenizer, model) for word in top_20_words}
similarities = {word: cosine_similarity(user_keyword_embedding, word_embeddings[word])[0][0] for word in top_20_words if word != user_keyword}

# 유사도가 높은 상위 5개 단어 선택
top_5_related_words = sorted(similarities, key=similarities.get, reverse=True)[:5]

# 결과 출력
print("연관 검색어로 사용할 상위 5개 단어:")
for word in top_5_related_words:
    print(f"{word}: {similarities[word]}")
